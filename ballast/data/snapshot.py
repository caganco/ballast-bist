"""Faz 0 frozen point-in-time price snapshot.

Freezes BIST100 + XU100 Close prices to a versioned parquet + metadata JSON with
a content hash, so every IC computation reads identical data (a prior check showed yfinance is non-reproducible: ~67% reconciliation). Close-only is sufficient for
...
and EXPLICITLY records the gap plus its bias DIRECTION (Cagan condition):
...
# ---------------------------------------------------------------------------
# Inlined from faz0_config.py (faz0_config taşınmıyor,
# sabitler burada inline edildi.)
# ---------------------------------------------------------------------------
SNAPSHOT_START = "2024-01-01"
SNAPSHOT_END = "2026-04-30"

FAZ0_ADV_MIN_DAYS = 60
...
FAZ0B_PAR_VALUE = 1.0
FAZ0B_PB_CROSSCHECK_TOL = 0.20
FAZ0B_BANKS = ("AKBNK", "ALBRK", "GARAN", "HALKB", "ISCTR", "SKBNK", "TSKB",
               "VAKBN", "YKBNK")
FAZ0B_FISCAL_YEARS = (2025, 2024, 2023, 2022)
...
def freeze_price_snapshot(
    ...
    adv_min_days: int = FAZ0_ADV_MIN_DAYS,
    tag: str = "",
    directive: str = "D-177",
    ...
    adv_floor_tl: if set, mechanically drop tickers whose median daily
    ...
    # Mechanical ADV liquidity trim (no judgmental exclusion).
...
# ===========================================================================
# Faz 0b: fundamental (MaliTablo) + FX (EVDS) snapshots
# ===========================================================================
...
def freeze_fundamental_snapshot(
    ...
    """Freeze (or load) per-ticker annual MaliTablo fundamentals.

    Idempotent. Banks (FAZ0B_BANKS) -> financialGroup=UFRS, book = total
    ...
    years = list(FAZ0B_FISCAL_YEARS)
    rows: list[dict] = []
    null_tickers: list[str] = []
    bank_n = 0
    for ticker in sorted(set(universe)):
        is_bank = ticker in FAZ0B_BANKS
        if is_bank:
            bank_n += 1
        try:
        ...
        for i, yr in enumerate(years):
            ...
            pub = (pd.Timestamp(period_end)
                   + pd.Timedelta(days=FAZ0B_ANNUAL_LAG_DAYS)).strftime("%Y-%m-%d")
            if is_bank:
    ...
    meta = {
        ...
        "fiscal_years": years, "annual_lag_days": FAZ0B_ANNUAL_LAG_DAYS,
        "itemcodes_nonbank": MALITABLO_ITEMCODES,
        "itemcodes_bank": MALITABLO_ITEMCODES_BANK,
        "coverage": {
            "requested_n": len(set(universe)),
            "loaded_n": int(df["ticker"].nunique()) if len(df) else 0,
            "null_tickers": sorted(null_tickers),
            "banks_n": bank_n, "banks": list(FAZ0B_BANKS),
        },
        "config_version": FAZ0B_CONFIG_VERSION,
    }
...
def _default_malitablo_fetch(ticker: str, fiscal_years: list[int], is_bank: bool) -> dict:
    ...
    group = MALITABLO_GROUP_BANK if is_bank else MALITABLO_GROUP_NONBANK
    codes = MALITABLO_ITEMCODES_BANK if is_bank else MALITABLO_ITEMCODES
...
def freeze_fx_snapshot(
    start: str = FAZ0B_WINDOW_START,
    end: str = FAZ0B_WINDOW_END,
    out_dir: Path | str = _SNAPSHOT_DIR,
    fetch_fn: Callable | None = None,
    tag: str = "faz0b",
) -> tuple[pd.Series, dict]:
    """Freeze (or load) EVDS USD/TRY (period-end) for USD ratios + sanity."""
    out_dir = Path(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    ...
    if fetch_fn is None:
        def fetch_fn(a, b):  # noqa: E731
            ...
            return fetch_series(EVDS_USDTRY_SERIES, start_date=a, end_date=b)
    ...
    meta = {"directive": "D-183", "series": EVDS_USDTRY_SERIES,
...
def freeze_par_guard(
    ...
    out_dir: Path | str = _SNAPSHOT_DIR,
    par: float = FAZ0B_PAR_VALUE,
    tol: float = FAZ0B_PB_CROSSCHECK_TOL,
    shares_fn: Callable | None = None,
    tag: str = "faz0b",
) -> dict:
    """Freeze the par!=1 / stale-shares NULL set.
...
def resolve_universe() -> list[str]:
    """v1 universe: config-driven 57 names. Fallback: BIST50."""
    try:
        from ballast.data.fetcher import get_bist100_tickers
...
def resolve_universe_v2() -> list[str]:
    """v2 candidate pool: BIST 100 index constituents (mechanical, no
    judgmental selection). ADV floor trims this pool mechanically at freeze."""
    return sorted(set(FAZ0_BIST100_CONSTITUENTS))
...
def _main() -> None:
    ...
    p.add_argument("--start", default=SNAPSHOT_START)
    p.add_argument("--end", default=SNAPSHOT_END)
    p.add_argument("--adv-floor", type=float, default=None,
                   help="median daily TL volume floor (v2 mechanical trim)")
